...
- **Charte** : reprise stricte de la charte NexusRH (orange `#FF5A1F` accent, noir `#0A0A0A`, gris `#F4F4F5`, typographie sans-serif moderne — `Inter` ou `Space Grotesk`).
...
- URL signée HMAC-SHA256, TTL 7 jours.
- Envoi par email via SendGrid (template responsive, plain-text alternatif).
...
3. Implémenter le seed (questions de l'Annexe 2 + settings).
4. Tests : modèles instanciables, migrations up/down idempotentes, seed re-exécutable.
...
2. Notebook `notebooks/train_models.ipynb` qui entraîne, valide (cross-val 5-fold), exporte `.joblib`.
3. Implémenter `services/ml_engine.py` (chargement, prédiction, garde-fous).
...
2. Validation Pydantic stricte de la sortie JSON.
3. Cache Redis, retry, fallback statique.
4. Tests avec mock de l'API Anthropic.

### Sprint 6 — API REST publique (2 jours)
1. Endpoints `/sessions`, `/responses`, `/finalize`, `/reports` (§6.1).
2. Rate limiting, validation Pydantic, gestion d'erreurs RFC 7807.
3. Tests d'intégration avec testcontainers PostgreSQL.

### Sprint 7 — Génération PDF (2 jours)
1. Template Jinja2 `templates/report.html` (§10.2) avec CSS print.
2. `services/report_service.py` : rendu WeasyPrint, stockage chiffré, URL signée.
3. Tests : 5 rapports de référence générés, comparaison de PDFs (texte extrait).

### Sprint 8 — Email (0,5 jour)
1. `services/email_service.py` (SendGrid ou SMTP), template HTML responsive.
2. Tests avec mock SMTP.

### Sprint 9 — Auth & Admin API (1,5 jour)
1. `/auth/login`, `/auth/refresh`, JWT.
2. Endpoints admin (§6.2).
3. Politique mot de passe, bruteforce protection.
4. Tests sécurité.

### Sprint 10 — Frontend public (3 jours)
1. Landing page.
2. Parcours questionnaire (1 question/écran, animations, auto-save).
3. Écran de génération avec polling.
4. Affichage rapport (jauges, charts).
5. Tests Vitest unitaires + 5 scénarios Playwright.

### Sprint 11 — Frontend admin (2 jours)
1. Login + dashboard.
2. CRUD questions / settings.
3. Stats, leads, audit logs.
4. Tests.

### Sprint 12 — Sécurité & conformité (1 jour)
1. Headers HTTP, CSP, HSTS.
2. Route `/privacy/erasure-request`.
3. Pages mentions légales, CGU.
4. Audit `pip-audit`, `npm audit`.

### Sprint 13 — CI/CD & déploiement (1 jour)
1. Workflows GitHub Actions complets.
...
3. Vérification couverture 100 %.
4. Documentation utilisateur (`README.md`, vidéo de démo).

**Total : ≈ 22 jours homme** — cohérent avec le planning du cahier des charges (livraison cible **30 juin 2026**).
...
- [ ] Le parcours visiteur complet (landing → questionnaire → rapport PDF reçu par email) fonctionne en moins de 6 minutes utilisateur.
- [ ] Les 20 questions du cahier des charges (Annexe 2) sont présentes avec les pondérations et types exacts.
- [ ] Le score global est calculé selon §8.2 et reproductible à la décimale près.
- [ ] Le rapport PDF respecte la charte NexusRH, fait 10-12 pages, est généré en ≤ 5 s.
- [ ] Les modèles ML atteignent les seuils §8.4 sur le set de validation.
- [ ] L'analyse Claude est strictement validée par Pydantic, le fallback fonctionne.
- [ ] Le compte admin permet d'éditer questions et taux sans redéploiement.
- [ ] **Couverture de tests = 100 %** sur les modules métier, **mutation score ≥ 95 %** sur les modules critiques.
- [ ] Les 8 scénarios E2E passent sur Chromium, Firefox, WebKit.
- [ ] Conformité ARTCI : mentions légales mot pour mot, consentement pré-décoché, route d'effacement opérationnelle.
- [ ] Headers de sécurité au niveau A+ sur securityheaders.com.
- [ ] CI bloque tout merge ne respectant pas les règles §12.4.
- [ ] Documentation : `README.md` complet, OpenAPI accessible, `DATASET.md` ML, runbook ops.
